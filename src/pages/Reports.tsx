                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ display: "flex" }}>
                        {(s.buyers || []).slice(0, 4).map((b, i) => (
                          <div key={b.name + i} className="spend-av" title={b.name}>{initials(b.name)}</div>
                        ))}
                      </div>
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const audits = auditRows.length ? auditRows : (sd || []).map((x: any, i: number) => ({
        ...
        revenueGross: Number((x as any).revenue_gross ?? x.revenue ?? 0),
        revenueNet: Number((x as any).revenue_net ?? x.revenue ?? 0),
        revenueGst: Number((x as any).revenue_gst ?? 0),
        tokenCollected: Number((x as any).token_collected ?? 0),
      ...
      const result: AttributionResult = {
        ...
        summary: {
          ...
          totalGrossRevenue: Number((session as any).total_gross_revenue ?? session.total_revenue ?? 0),
          totalNetRevenue: Number((session as any).total_net_revenue ?? session.total_revenue ?? 0),
          totalRevenueGst: Number((session as any).total_revenue_gst ?? 0),
          totalTokenCollected: Number((session as any).total_token_collected ?? 0),
          totalAdSpend: Number(session.total_ad_spend),
          overallRoas: Number(session.overall_roas),
        },
        revenueConfig: (session as any).revenue_mode ? {
          mode: (session as any).revenue_mode,
          productPrice: Number((session as any).product_price ?? 0),
          productName: (session as any).product_name || "",
          productGstMode: (session as any).product_gst_mode || "inclusive",
          productGstPercent: Number((session as any).product_gst_percent ?? 18),
          roasRevenueBasis: (session as any).roas_revenue_basis || "gross",
        } : null,
        mediaBuyerBreakdown,
