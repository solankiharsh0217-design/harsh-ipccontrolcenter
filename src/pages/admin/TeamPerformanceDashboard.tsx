...
export default function TeamPerformanceDashboard() {
  ...
  return (
    <div className="max-w-[1280px]">
      ...
      {/* Filters */}
      <div className="rounded-xl border border-line bg-white px-4 py-3.5 mb-6 flex flex-wrap gap-2 items-center">
        <div className="flex gap-1">
          {(["today", "yesterday", "week", "month"] as RangePreset[]).map((p) => (
            <button
              key={p}
              onClick={() => applyPreset(p)}
              className={`font-sans text-[11px] font-medium px-2.5 py-1.5 rounded-md border transition-colors ${filters.preset === p ? "bg-black text-white border-black" : "bg-white text-black border-line hover:border-black"}`}
            >
        ...
        <div className="flex gap-1 items-center ml-1">
          <input type="date" value={filters.from} onChange={(e) => setFilters((f) => ({ ...f, preset: "custom", from: e.target.value }))} className="font-sans text-[11px] border border-line rounded-md px-2 py-1.5" />
          <span className="text-muted-foreground text-[11px]">→</span>
          <input type="date" value={filters.to} onChange={(e) => setFilters((f) => ({ ...f, preset: "custom", to: e.target.value }))} className="font-sans text-[11px] border border-line rounded-md px-2 py-1.5" />
        </div>
        <select value={filters.userId} onChange={(e) => setFilters((f) => ({ ...f, userId: e.target.value }))} className="font-sans text-[11px] border border-line rounded-md px-2 py-1.5 bg-white">
          <option value="">All team members</option>
          {members.map((m) => <option key={m.id} value={m.id}>{m.full_name || m.email}</option>)}
        </select>
        <select value={filters.role} onChange={(e) => setFilters((f) => ({ ...f, role: e.target.value }))} className="font-sans text-[11px] border border-line rounded-md px-2 py-1.5 bg-white">
          <option value="">All roles</option>
          {uniqueRoles.map((r) => <option key={r} value={r}>{r}</option>)}
        </select>
        <select value={filters.cadence} onChange={(e) => setFilters((f) => ({ ...f, cadence: e.target.value }))} className="font-sans text-[11px] border border-line rounded-md px-2 py-1.5 bg-white">
        ...
        <select value={filters.status} onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value }))} className="font-sans text-[11px] border border-line rounded-md px-2 py-1.5 bg-white">
      ...
      {error && <div className="mb-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 font-sans text-[12px] text-red-700">{error}</div>}
      ...
      <div className="rounded-xl border border-line bg-white px-4 py-3 mb-8">
        ...
        <div className="w-full h-2 rounded-full bg-off overflow-hidden">
          <div className="h-full bg-black transition-all" style={{ width: `${Math.min(100, kpiSummary.rate)}%` }} />
        </div>
      </div>

      {/* Scores + Reward liability */}
                  ...
                  {attention.slice(0, 20).map((r) => (
                    <tr key={r.id} className="border-t border-line">
                      ...
                      <td className="px-3 py-2 text-right">
                        <button onClick={() => nav("/team-performance/review")} className="text-[11px] font-medium underline underline-offset-2 hover:text-black">Review →</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  {filteredPendingSubs.slice(0, 10).map((s) => {
                    ...
                    return (
                      <tr key={s.id} className="border-t border-line">
                        ...
                        <td className="px-3 py-2 text-right">
                          <button onClick={() => nav("/team-performance/review")} className="text-[11px] font-medium underline underline-offset-2 hover:text-black">Open →</button>
                        </td>
                      </tr>
                    );
                  })}
      ...
      <div className="rounded-xl border border-line bg-white overflow-hidden mb-10">
              ...
              {filteredMembers.map((m) => {
                ...
                return (
                  <tr key={m.id} className="border-t border-line">
                    ...
                    <td className="px-3 py-2 text-right text-emerald-700">{cnt("approved")}</td>
                    <td className="px-3 py-2 text-right">{cnt("pending")}</td>
                    <td className="px-3 py-2 text-right">{cnt("submitted")}</td>
                    <td className="px-3 py-2 text-right text-red-700">{cnt("rejected")}</td>
                    <td className="px-3 py-2 text-right text-amber-700">{cnt("missed")}</td>
                    ...
                    <td className="px-3 py-2 text-right">
                      <button onClick={() => nav("/team-performance/review")} className="text-[11px] font-medium underline underline-offset-2 hover:text-black">Scorecard →</button>
                    </td>
                  </tr>
                );
              })}
