...
import { X, Phone, MessageCircle, Mail, MessageSquare, Trash2, ExternalLink, Sparkles, ChevronDown, Archive, RotateCcw, Plus, CreditCard, Pencil, Check, Settings, ShieldAlert, History, Loader2 } from "lucide-react";
...
export default function LeadDrawer({ leadId, stages, agents, onClose, onChanged, onStageChanged, originLabel, returnTo }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30" onClick={onClose}>
      <div className="absolute right-0 top-0 h-full w-[560px] bg-background border-l border-line flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
              ...
              {editMode ? (
                ...
                <>
                  <div className="flex items-center gap-2 mb-1.5">
                    <div className="font-serif text-[22px] leading-tight text-foreground">{lead.full_name || "Unnamed"}</div>
                    <button onClick={() => { setEditName(lead.full_name || ""); setEditEmail(lead.email || ""); setEditPhone(lead.phone || ""); setEditMode(true); }} className="p-1 hover:bg-off rounded text-muted-foreground"><Pencil className="w-3.5 h-3.5" /></button>
                  </div>
                  <div className="flex flex-col gap-1.5 text-xs text-muted-foreground">
                    <div className="flex items-center gap-4">
                      {lead.phone && <div className="flex items-center gap-1.5"><Phone className="w-3 h-3" /> <span>{lead.phone}</span></div>}
                      {lead.email && <div className="flex items-center gap-1.5"><Mail className="w-3 h-3" /> <span>{lead.email}</span></div>}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-1.5 mt-4">
                    <span className="px-2 py-0.5 rounded-full text-[10px] uppercase font-medium bg-off border border-line text-muted-foreground">{lead.program_name}</span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] uppercase font-medium bg-off border border-line text-muted-foreground">{lead.lead_type}</span>
                  </div>
                </>
              )}
            </div>
            <button onClick={onClose} className="p-2 hover:bg-off rounded text-muted-foreground"><X className="w-4 h-4" /></button>
          </div>
        </div>

        <div className="flex-1 min-h-0 overflow-y-auto">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
            <div className="sticky top-0 z-10 px-6 border-b border-line bg-background">
              <TabsList className="bg-transparent h-12 w-full justify-start overflow-x-auto no-scrollbar gap-6">
                <TabsTrigger value="overview" className="text-xs font-medium px-0 pb-3 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-black rounded-none">Overview</TabsTrigger>
                <TabsTrigger value="payments" className="text-xs font-medium px-0 pb-3 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-black rounded-none">Payments</TabsTrigger>
                <TabsTrigger value="onboarding" className="text-xs font-medium px-0 pb-3 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-black rounded-none">Onboarding</TabsTrigger>
                <TabsTrigger value="stage & handoff" className="text-xs font-medium px-0 pb-3 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-black rounded-none">Stage & Handoff</TabsTrigger>
                <TabsTrigger value="follow-ups & activity" className="text-xs font-medium px-0 pb-3 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-black rounded-none">Activity</TabsTrigger>
              </TabsList>
            </div>
