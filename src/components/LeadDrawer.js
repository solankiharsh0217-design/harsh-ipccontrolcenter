...
import { getActiveHandoffRules, findRuleForStage } from "@/lib/operationsCrm";
...
import { loadActiveConversionRules } from "@/lib/conversionRules";
...
export default function LeadDrawer({ leadId, stages, agents, onClose, onChanged, onStageChanged, originLabel, returnTo }) {
    const { profile, isAdmin } = useAuth();
    const [lead, setLead] = useState(null);
    const [activities, setActivities] = useState([]);
    const [reminders, setReminders] = useState([]);
    const [paidSnap, setPaidSnap] = useState(null);
    const [opsLeadId, setOpsLeadId] = useState(null);
    ...
    const [convRules, setConvRules] = useState([]);
    const [opsRules, setOpsRules] = useState([]);
    const [archiveOpen, setArchiveOpen] = useState(false);
    const [archiveBusy, setArchiveBusy] = useState(false);
    const [archiveLinks, setArchiveLinks] = useState(null);
    const [deleteOpen, setDeleteOpen] = useState(false);
    const [deleteBusy, setDeleteBusy] = useState(false);
    const [deleteBlocked, setDeleteBlocked] = useState(null);
    const [openPay, setOpenPay] = useState(false);
    const [payPrefill, setPayPrefill] = useState(null);
    const [payHeaderNote, setPayHeaderNote] = useState(undefined);
    const [postPayAction, setPostPayAction] = useState(null);
    ...
    const load = async () => {
        ...
        setActivities((a || []));
        setReminders((r || []));
        const ppid = l?.paid_pipeline_lead_id;
        if (ppid) {
    ...
    const pipelineStages = lead ? stages.filter((s) => s.pipeline_id === lead.pipeline_id).filter((s) => s.is_active !== false || s.id === lead.stage_id) : [];
    ...
    const paidLeadId = lead ? lead.paid_pipeline_lead_id : null;
    const handlePaymentSaved = async () => {
        const paidLeadId = lead.paid_pipeline_lead_id;
        if (paidLeadId && postPayAction === "setTokenPaid") {
            await supabase.from("paid_pipeline_leads").update({ pipeline_stage: "Token Paid" }).eq("id", paidLeadId);
    ...
    const primaryAction = useMemo(() => {
        ...
        const activeConvRule = convRules.find(r => {
            if (r.source_pipeline_id && r.source_pipeline_id !== lead.pipeline_id)
                return false;
            if (r.trigger_stage_ids?.includes(lead.stage_id))
                return true;
            if (currentStageName && r.trigger_stage_names?.some(n => n.toLowerCase() === currentStageName.toLowerCase()))
                return true;
            return false;
        });
        if (activeConvRule) {
    ...
    useEffect(() => {
        ...
        if (reminders.some(r => r.reminder_date < today)) {
    ...
    return (!lead ? (<div className="fixed inset-0 z-50 bg-black/30" onClick={onClose}>
      <div className="absolute right-0 top-0 h-full w-[560px] bg-white p-8" onClick={(e) => e.stopPropagation()}>Loading</div>
        ...
        if (prev !== stageId) {
            ...
            try {
                ...
                await evaluateStageTrigger({
                    ...
                    crmLeadId: lead.id,
                    paidPipelineLeadId: lead.paid_pipeline_lead_id || null,
        ...
        setAddingStage(true);
        try {
            const { data, error } = await supabase.from("stages").insert({ pipeline_id: lead.pipeline_id, name, color: "gray", position: pipelineStages.length }).select("id").maybeSingle();
            if (error) {
    ...
    return (<div className="fixed inset-0 z-50 bg-black/30" onClick={onClose}>
      <div className="absolute right-0 top-0 h-full w-[560px] bg-white border-l border-line flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
            ...
            <TabsContent value="payments" className="flex-1 overflow-y-auto p-6">
              {(paidSnap || Number(lead.deal_value) > 0) && (<div className="rounded-xl border border-line bg-card p-5 space-y-6">
                  ...
                  {paidLeadId && (<div className="flex gap-2">
                      {!hasToken ? (<button onClick={openTokenPayment} className="h-9 px-4 bg-black text-white rounded-md text-sm font-medium hover:bg-black/90">Record Token</button>) : (<button onClick={() => { setPayPrefill(null); setPostPayAction(null); setOpenPay(true); }} className="h-9 px-4 border border-line rounded-md text-sm font-medium hover:bg-off">Add Payment</button>)}
                    </div>)}
                </div>)}
            </TabsContent>
            ...
            <TabsContent value="onboarding" className="flex-1 overflow-y-auto p-6 space-y-8">
              <section className="space-y-4">
                ...
                <AccessVerificationPanel memberLabel={lead.full_name || undefined} crmLeadId={lead.id} cocStatus={lead.code_of_conduct_status}/>
              </section>
            ...
            <TabsContent value="stage & handoff" className="flex-1 overflow-y-auto p-6 space-y-8">
              <section className="space-y-4">
                ...
                <CrmStagePicker stages={pipelineStages} currentStageId={lead.stage_id} open={crmPickerOpen} onOpenChange={setCrmPickerOpen} newStageName={newStageName} onNewStageNameChange={setNewStageName} onAddStage={addStageInline} onChangeStage={(id) => { moveStage(id); setCrmPickerOpen(false); }} onDeleteStage={() => { }}/>
              </section>

              {isOpsEligible && !inOps && (<section className="p-4 rounded-xl border border-amber-200 bg-amber-50 space-y-3">
            ...
            <TabsContent value="follow-ups & activity" className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-4">
                  {activities.length === 0 ? (<div className="text-sm text-muted-foreground italic px-2">No activity recorded yet.</div>) : (activities.map(a => (<div key={a.id} className="relative pl-6 pb-4 border-l border-line last:border-0 last:pb-0">
        ...
        <div className="px-6 py-4 border-t border-line flex items-center justify-between bg-white shadow-sm">
          ...
          <div className="flex items-center gap-3">
            {primaryAction && (<button onClick={primaryAction.onClick} className={`h-10 px-6 rounded-md text-sm font-medium transition-all ${primaryAction.variant === "black"
                ? "bg-black text-white hover:bg-black/90 shadow-md active:scale-[0.98]"
                : "border border-line text-foreground hover:bg-off"}`}>
                {primaryAction.label}
              </button>)}
            <button onClick={() => { load(); onChanged(); onClose(); }} className="h-10 px-6 bg-black text-white rounded-md text-sm font-medium hover:bg-black/90 shadow-md active:scale-[0.98]">
              Save & Close
            </button>
          </div>
        </div>
      ...
      {sendOpsOpen && <SendToOperationsCrmModal candidateLeads={[lead]} sourceStages={[]} preSelectedIds={[lead.id]} prefill={rulePrefill} onClose={() => setSendOpsOpen(false)} onDone={() => { setSendOpsOpen(false); load(); }}/>}
      ...
      {convertOpen && <ConvertToPaidModal lead={lead} agents={agents} onClose={() => setConvertOpen(false)} onConverted={() => { setConvertOpen(false); load(); onChanged(); }}/>}
      {sendOnboardingOpen && (<SendToPaidOnboardingModal open={sendOnboardingOpen} onClose={() => setSendOnboardingOpen(false)} leadId={lead.id} leadName={lead.full_name || ""} leadEmail={lead.email} leadPhone={lead.phone} onDone={() => { setSendOnboardingOpen(false); load(); onChanged(); }}/>)}
