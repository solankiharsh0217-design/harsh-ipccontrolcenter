...
export const EmptyState = ({
  ...
  <div className={`font-sans text-muted-foreground ${bordered ? "border border-dashed border-line rounded-lg py-12 px-4 text-center text-sm" : `text-[12px] py-6 ${center ? "text-center" : ""}`} ${className}`}>
    <div>{title}</div>
    {hint && <div className="text-[12px] opacity-80 mt-1">{hint}</div>}
  </div>
);
