...
BEGIN
  ...
  -- Subtransaction: any UPDATE here is rolled back by RAISE EXCEPTION below.
  BEGIN
    UPDATE public.leads
       SET archived_at = now()
     WHERE id = v_lead_id;
    ...
    v_result := jsonb_build_object(
      'passed', (v_new_archived IS NOT NULL
                 AND v_lead_type = 'paid'
                 AND v_paid_link IS NOT NULL),
      'lead_id', v_lead_id,
    ...
    -- Force rollback of the UPDATE while keeping v_result captured above.
    RAISE EXCEPTION 'ROLLBACK_TEST' USING ERRCODE = 'P0001';
  EXCEPTION WHEN SQLSTATE 'P0001' THEN
    -- Expected — the UPDATE has been rolled back.
    NULL;
  END;
